...
spark = SparkSession.builder \
    .appName('PM25StreamingCassandra') \
...
def save_to_cassandra(rows):
    ...
    session = cluster.connect('pm25')
    for row in rows:
        session.execute("""
            INSERT INTO forecast_results (month, avg_pm25_actual, forecast_next_month)
            VALUES (%s, %s, %s)
        """, (row['month'], row['avg_pm25_actual'], row['forecast_next_month']))
    cluster.shutdown()
    print(f"Da luu {len(rows)} dong vao Cassandra!")
